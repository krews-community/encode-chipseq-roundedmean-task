...
class TestCmdRunner : CmdRunner {
    override fun run(cmd: String) = exec("docker", "exec", "chipseq-roundedmean-base", "sh", "-c", cmd)
    override fun runCommand(cmd: String):String? = getCommandOutput("docker", "exec", "chipseq-roundedmean-base", "sh", "-c", cmd)
